...
import { Table, TableColumn, Progress, ResponseErrorPanel, InfoCard, StatusOK, StatusError, StatusWarning, Link } from '@backstage/core-components';
...
type Impact = 'OPERATIONAL' | 'UNDERMAINTENANCE' | 'DEGRADEDPERFORMANCE' | 'PARTIALOUTAGE' | 'MAJOROUTAGE';
...
export const CardView = ({ pageId, status, reportUrl }: DenseTableProps) => {
    ...
    const incidents = status.activeIncidents?.map(incident => {
        ...
        return {
            status: <Status>{incident.status} - {impact[incident.impact] || ''}</Status>,
    ...
    return (
        <InfoCard title="Current Status">
            <div className={classes.wrapper}>
                ...
                {status.page.status === 'UP' && <StatusOK>Everything is fine!</StatusOK>}
                {status.page.status === 'UNDERMAINTENANCE' && <StatusWarning>We are performing routine maintenance</StatusWarning>}
                {status.page.status === 'HASISSUES' && <StatusError>We are having issues!</StatusError>}
            </div>
            {!!incidents && <Table
            ...
            />}
            <div className={classes.wrapper}>
                <Button variant="outlined" className={classes.actionButton} href={`https://${pageId}.instatus.com`} target="_blank">

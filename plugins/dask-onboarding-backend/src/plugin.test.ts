...
import { daskOnboardingBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [daskOnboardingBackendPlugin],
    });

    await request(server).get('/api/dask-onboarding-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/dask-onboarding-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/dask-onboarding-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/dask-onboarding-backend/todos/${createdTodoItem.id}`)

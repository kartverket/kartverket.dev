...
//
//   curl http://localhost:7007/api/dask-onboarding-backend/todos -H 'Content-Type: application/json' -d '{"title": "My Todo"}'
//   curl http://localhost:7007/api/dask-onboarding-backend/todos -H 'Content-Type: application/json' -d '{"title": "My Todo", "entityRef": "component:default/sample"}'
//
// List TODOs:
//
//   curl http://localhost:7007/api/dask-onboarding-backend/todos
//
// Explicitly make an unauthenticated request, or with service auth:
//
//   curl http://localhost:7007/api/dask-onboarding-backend/todos -H 'Authorization: Bearer mock-none-token'
//   curl http://localhost:7007/api/dask-onboarding-backend/todos -H 'Authorization: Bearer mock-service-token'
    ...
    entities: [
      {
        ...
        kind: 'Component',
        metadata: {
          name: 'dask-onboarding-backend',
          title: 'DASK Onboarding Backend',
        },
        spec: {
          type: 'service',
        },

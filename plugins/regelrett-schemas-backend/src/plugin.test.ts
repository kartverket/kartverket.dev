...
import { regelrettSchemasPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [regelrettSchemasPlugin],
    });

    await request(server).get('/api/regelrett-schemas/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/regelrett-schemas/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/regelrett-schemas/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/regelrett-schemas/todos/${createdTodoItem.id}`)
  ...
  it('should forward errors from the TodoListService', async () => {
    ...
    const listRes = await request(server).get('/api/regelrett-schemas/todos');
    expect(listRes.status).toBe(401);
    expect(listRes.body).toMatchObject({
      error: { name: 'AuthenticationError' },
    });

    const getRes = await request(server).get('/api/regelrett-schemas/todos/123');
